...
export class ProductCardOneComponent implements OnInit {
  ...
  getCompareListFromDB() {
    this.productService.getSpecificProductsById(this.compareListV2, 'productName attributes productSlug category price discountType discountAmount quantity images shortDescription')
      .subscribe( res => {
        this.spinner.hide();
